...
// FINE_GRAIN_SIZE
//    Fine-grained file range locks are implemented by splitting
//    the file into regions, each of size FILE_GRAIN_SIZE, and 
//    locking only those that overlap the request.

#define FINE_GRAIN_SIZE 16


// region_lock
//    Data structure for a region; used to implement fine-grained locking.

struct region_lock
{
	unsigned locked = 0;	// unlocked if 0, blocked by owner if != 0
	std::thread::id owner;	// thread ID of owner if != 0
};
...
struct io61_file 
{
	...
	// For multi-threading
	std::recursive_mutex m;			// mutex protecting file's state
	std::condition_variable_any cv;	// used to wake up waiting threads;
	region_lock* reg;				// dynamically allocated array of regions,
									// each FINE_GRAIN_SIZE bytes
	int nreg = -1;					// number of regions
};


// file_region(off)
//    Returns the index of the region in 'reg' which contains the
//    position represented by 'off'.

static int file_region(off_t off) 
{
    return off / FINE_GRAIN_SIZE;
}
...
io61_file* io61_fdopen(int fd, int mode) 
{
	...
	// dynamically allocate array of regions for fine-graining
	off_t filesize = io61_filesize(f);
	if (filesize > 0)
	{
		f->nreg = io61_filesize(f) / FINE_GRAIN_SIZE + 1;
		f->reg = new region_lock[f->nreg];
	}
...
int io61_close(io61_file* f) 
{
	...
	// dynamically deallocate array of regions if it exists
	if (f->nreg != -1)
		delete f->reg;
...
ssize_t io61_pread(io61_file* f, 
				   unsigned char* buf, 
				   size_t sz,
                   off_t off)
{
...
ssize_t io61_pwrite(io61_file* f, 
...
int io61_try_lock(io61_file* f, off_t start, off_t len, int locktype)
{
	...
	// acquire guard, fail if locked by another thread
    std::unique_lock guard(f->m);
    if (may_overlap_with_other_lock(f, start, len))
        return -1;

	// success; account for new lock
...
int io61_lock(io61_file* f, off_t start, off_t len, int locktype) 
{
	...
	// acquire guard, wait w/ cv until not locked by another thread
	std::unique_lock guard(f->m);
...
int io61_unlock(io61_file* f, off_t start, off_t len)
{
	...
	// acquire guard; account for unlock; wake waiting threads
	std::unique_lock guard(f->m);
	int rstart = file_region(start), rend = file_region(start + len - 1);
	for (int ri = rstart; ri <= rend; ++ri)
		--f->reg[ri].locked;
	f->cv.notify_all(); // spurious wakeup possible, but as per the
						// provided hints we'll consider it 'no big deal'
	return 0;
}
